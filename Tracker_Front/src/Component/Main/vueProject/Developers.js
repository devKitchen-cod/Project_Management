					...
					{users.map((item) => (
						<Card className="some123123">
							...
							<Card.Body>
								...
								<div>My stack: {item.developers_stack}</div>
								<div>My sallary: {item.sallary}$/hr</div>
								<div>My current project: {item.current_project}</div>
							</Card.Body>

							<Card.Footer>Get contackt with me: {item.email_user}</Card.Footer>
						</Card>
					))}
				</Tab>
