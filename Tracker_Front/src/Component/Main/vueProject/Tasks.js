...
export default function Tasks() {
	...
	useEffect(() => {
		dispatch(reqreadtask(readedTasks));
	}, [readedTasks]);
	useEffect(() => {
		dispatch(reqreadplannedTaskF(readplannedTask, readedTasks));
	}, []);
	useEffect(() => {
		dispatch(reqreadInProgresTaskF(readInProgresTask, readedTasks));
	}, []);
	useEffect(() => {
	...
	const readed_inprogress_tasks = useSelector(
		(state) => state.setTask.inprogress
	);
	const readed_done_tasks = useSelector((state) => state.setTask.done);

	useEffect(() => {
		if (state.reducer.deleted == true) {
			dispatch(reqreadtask(readedTasks));
		}
						...
						<Card className="tab-plan">
							<Card.Body>
								...
								<Card.Subtitle className="mb-2 text-muted">
									<h5>Time:</h5> {item.planned_time_task}
								</Card.Subtitle>
								<Card.Text>
									<Accordion defaultActiveKey="0">
										<Accordion.Item eventKey="1">
											<Accordion.Header>Description</Accordion.Header>
											<Accordion.Body>{item.description_task}</Accordion.Body>
										</Accordion.Item>
									</Accordion>
								</Card.Text>
							</Card.Body>
							<Card.Footer className="footer">
								<Dropdown className="btn">
									...
									<Dropdown.Menu>
										...
										<Dropdown.Item>
											<Button onClick={() => handle_Done(item.id)}>Done</Button>
										</Dropdown.Item>
										<Dropdown.Item>
											<Button onClick={() => handle_InProress(item.id)}>
												In proress
											</Button>
										</Dropdown.Item>
										<Dropdown.Divider />
						...
						<Card className="tab-plan">
							...
							<Card.Footer className="footer">
								<Dropdown className="btn">
									...
									<Dropdown.Menu>
										<Dropdown.Item>
											<Button onClick={() => handle_Planned(item.id)}>
												Planned
											</Button>
										</Dropdown.Item>
